...
for (const [yearText, expected] of Object.entries(supportedCounts)) {
  ...
  for (const [scope, expectedCount] of Object.entries(expected)) {
    ...
    assert.strictEqual(calculable.length, automaticCounts[scope], `${year}年度${scope}は条件表・補正式まで構造化済みの項目だけを計算入力可能にする`);
    assert.strictEqual(rules.length - calculable.length, expectedCount - automaticCounts[scope], `${year}年度${scope}の未構造化条件付き項目を安全停止する`);
  }
}
