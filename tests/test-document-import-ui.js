...
assert.ok(ui.includes("openPdfCandidateChoice") && ui.includes("candidateChoiceHtml") && ui.includes("applyPdfCandidateChoice"), "複数候補は通知だけで終わらず一覧から選んで入力欄へ反映できる");
assert.strictEqual((ui.match(/openPdfCandidateChoice\(lineId, targetType, matched\.matches\)/g) || []).length, 2, "測量と設計等の両方で複数候補一覧を開く");
assert.ok(ui.includes('targetType === "item" ? "survey"') && ui.includes("businessKindLabel(choice.kind)") && ["設計業務", "測量業務", "調査・計画業務", "地質業務"].every((label) => ui.includes(label)), "候補選択を4業務共通の画面として扱う");
assert.ok(html.includes("この操作だけでは積算へ追加しません") && ui.includes("入力欄へ反映しました"), "候補選択は作業項目の入力までとし積算へ自動追加しない");
assert.ok(css.includes(".pdf-candidate-choice-dialog") && css.includes(".pdf-candidate-choice-list") && css.includes(".pdf-candidate-choice-scope"), "候補一覧を読みやすい選択画面で表示する");
assert.ok(html.includes('id="clearPdfManualLineSelectionButton"') && html.includes("PDF行の選択を解除") && !html.includes("この行は対象外") && !ui.includes("ignoredPdfLines"), "対象外指定ではなく入力途中のPDF行選択を解除する");
assert.ok(ui.includes('$("clearPdfManualLineSelectionButton").disabled = false;') && ui.includes('$("clearPdfManualLineSelectionButton").disabled = true;') && css.includes("#clearPdfManualLineSelectionButton:disabled"), "PDF行解除は入力中だけ有効にし、それ以外は灰色で無効化する");
assert.ok(ui.includes("if (!currentManualLineId || currentEditingTarget) return;") && ui.includes("draggedPdfLines.delete(lineId)"), "解除時は入力途中のPDF行だけを未選択へ戻し、反映待ち編集中は変更しない");
const itemDropIndex = html.indexOf('data-pdf-drop-target="item"');
const unitDropIndex = html.indexOf('data-pdf-drop-target="unit"');
const quantityDropIndex = html.indexOf('data-pdf-drop-target="quantity"');
assert.ok(itemDropIndex >= 0 && itemDropIndex < unitDropIndex && unitDropIndex < quantityDropIndex, "測量PDF入力は作業項目、単位、数量の順に表示する");
assert.ok(html.includes('<b aria-hidden="true">2</b>PDFの単位をドロップ') && html.includes('<b aria-hidden="true">3</b>PDFの数量をドロップ'), "測量PDF入力の番号を1・2・3へ揃える");
...
assert.ok(html.includes('<span class="sr-only">積算基準の作業区分</span>') && html.includes('<span class="sr-only">資料の単位・数量</span>') && html.includes('id="pdfManualSurveyConversion" hidden') && html.includes('id="pdfManualSurveyRule" hidden'), "不要な補助見出しと未選択時メッセージを画面へ表示しない");
